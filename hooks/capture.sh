#!/usr/bin/env bash
# Celeborn Stop hook — collapsed (executable-app.md §3). Thin shim that execs the in-process
# `celeborn hook stop` entry point. Reads the host's event JSON on stdin (inherited via exec) and
...
exec python3 "$DIR/scripts/celeborn.py" hook stop
