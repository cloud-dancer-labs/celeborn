#!/usr/bin/env bash
# Celeborn statusLine hook — collapsed (executable-app.md §3). Thin shim that execs the in-process
# `celeborn hook statusline` entry point. Reads the host's event JSON on stdin (inherited via exec) and
...
exec python3 "$DIR/scripts/celeborn.py" hook statusline
