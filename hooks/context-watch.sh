#!/usr/bin/env bash
# Celeborn UserPromptSubmit hook — collapsed (executable-app.md §3). Thin shim that execs the in-process
# `celeborn hook user-prompt-submit` entry point. Reads the host's event JSON on stdin (inherited via exec) and
...
exec python3 "$DIR/scripts/celeborn.py" hook user-prompt-submit
