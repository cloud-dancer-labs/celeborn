#!/usr/bin/env bash
# Celeborn SessionEnd hook — collapsed (executable-app.md §3). Thin shim that execs the in-process
# `celeborn hook session-end` entry point. Reads the host's event JSON on stdin (inherited via exec) and
...
exec python3 "$DIR/scripts/celeborn.py" hook session-end
